...
session_start();
$session_promo = 'promo1'; // . $_POST['promo_id'];
...
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอก เบอร์โทรศัพท์', 'error');
</script>";
} elseif (empty($_POST['password'])) {
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอก รหัสผ่าน', 'error');
</script>";
} elseif (empty($_POST['firstname'])) {
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอก ชื่อจริง', 'error');
</script>";
} elseif (empty($_POST['lastname'])) {
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอก นามสกุล', 'error');
</script>";
} elseif (empty($_POST['bankaccount'])) {
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอก หมายเลขบัญชี', 'error');
</script>";
} elseif (empty($_POST['bankcode'])) {
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณาเลือก ธนาคาร', 'error');
</script>";
} elseif (empty($_POST['lineid'])) {
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอก ไอดีไลน์', 'error');
</script>";
} elseif (empty($_POST['captcha'])) {
echo "<script>
    swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอก CAPTCHA', 'error');
</script>";
} else {
...
if ($count_ip['ip'] > 0) {
echo "<script>
    swal.fire('ข้อมูลซ้ำ', 'หมายเลข IP ของคุณเคยสมัครไปแล้ว', 'error');
</script>";
exit();
}
...
echo "<script>
    swal.fire('ข้อมูลไม่ถูกต้อง', 'หมายเลขโทรศัพท์ ต้องเป็นตัวเลข 10 หลักเท่านั้น', 'error');
</script>";
} elseif (mb_strlen($password, 'UTF8') < 8) { echo "<script>swal.fire('ข้อมูลไม่ถูกต้อง','รหัสผ่านต้องมีไม่ต่ำกว่า 8 ตัว', 'error');</script>" ; } elseif (!preg_match($re, $password)) { if (misc_parsestring($password)==false) { echo "<script>swal.fire('ข้อมูลไม่ถูกต้อง','รหัสผ่านต้องมีอักษรภาษาอังกฤษกับตัวเลขผสมเท่านั้น', 'error');</script>" ; } else { echo "<script>swal.fire('ข้อมูลไม่ถูกต้อง','รหัสผ่านต้องมีตัวเลขอยู่ด้วยอย่างน้อย 1 ตัว', 'error');</script>" ; } } elseif (!preg_match('/^[ก-๙เ]+$/', $name)) { echo "<script>swal.fire('ข้อมูลไม่ถูกต้อง','ชื่อจริงต้องเป็นภาษาไทยเท่านั้น ห้ามมีเว้นวรรค', 'error');</script>" ; } elseif (!preg_match('/^[ก-๙เ]+$/', $surname)) { echo "<script>swal.fire('ข้อมูลไม่ถูกต้อง','นามสกุลต้องเป็นภาษาไทยเท่านั้น ห้ามมีเว้นวรรค', 'error');</script>" ; } elseif (misc_parsestring($banknumber, '0123456789' )==false) { echo "<script>swal.fire('ข้อมูลไม่ถูกต้อง','หมายเลขบัญชี ต้องเป็นตัวเลข 10 หลักเท่านั้น', 'error');</script>" ; } elseif ($row_bank> 0) {
    echo "<script>
        swal.fire('ข้อมูลซ้ำ', 'หมายเลขบัญชี $banknumber นี้ถูกใช้งานแล้ว', 'error');
    </script>";
    } elseif ($row_phone > 0) {
    echo "<script>
        swal.fire('ข้อมูลซ้ำ', 'หมายเลขโทรศัพท์: $phone นี้ถูกใช้งานแล้ว', 'error');
    </script>";
    } elseif ($row_line > 0) {
    echo "<script>
        swal.fire('ข้อมูลซ้ำ', 'ไลน์ไอดี: $lineid นี้ถูกใช้งานแล้ว', 'error');
    </script>";
    } elseif ($_SESSION['captcha']['code'] <> $captcha_code) {
        echo "<script>
            swal.fire('ข้อมูลผิด', 'CAPTCHA ไม่ถูกต้อง', 'error');
        </script>";
        } else {
