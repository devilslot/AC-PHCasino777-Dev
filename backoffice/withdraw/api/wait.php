...
if($_GET['id'] != ''){
    ...
    $data = $mysqli->query("SELECT * FROM `slot_withdraw` WHERE `wd_status` = 0 AND mod(wd_id,".$count['count'].") = ".$mod)->fetchAll();
}else{
...
<tr>
    ...
    <?php $count = $mysqli->query("SELECT bank_id FROM `slot_bank_transfer` WHERE bank_status = 1")->fetchAll();
        ...
        <button class="btn btn-success btn-sm"
            ...
            onclick="req_otp(<?php echo '\'' . $user['member_bank_number'] . '\',\'' . bank_type_rename($user['member_bank_type']) . '\',\'' . $result['wd_amount'] . '\',\'' . $result['wd_id'] . '\','.$count[$arr]['bank_id'].'' ?>)">

            <i class="fa fa-check"></i>SCB <?php echo $i;?></button>
        <?php } ?>
