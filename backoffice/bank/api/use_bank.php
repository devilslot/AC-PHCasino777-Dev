...
if(isset($_POST['username'])){
    ...
    $trans = $mysqli->query("SELECT * FROM scb_transaction_".$_POST['id']." WHERE trans_no = ? ",$_POST['trans_no'])->fetchArray();
    ...
    if(isset($user['member_level'])){
        ...
        if(isset($credit->Credit)){
            ...
            $mysqli->query("INSERT INTO `slot_topup` (`topup_username`,`topup_type`,`topup_amount`,`topup_credit`,`topup_datetime`,`topup_bonus`,`topup_info`,`topup_ocode`) VALUES (?,?,?,?,?,?,?,?)",$send['username'],'promptpay',$trans['trans_amount'],$credit_all,date('Y-m-d H:i:s'),$bonus,$_POST['info'],'');

            $mysqli->query("UPDATE scb_transaction_".$_POST['id']." SET trans_status = 1,trans_used_username = ? WHERE trans_no = ?",$send['username'],$_POST['trans_no']);
