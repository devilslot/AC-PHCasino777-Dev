                            ...
                            <div class="card">
                                ...
                                <div class="card-header">
                                    ...
                                    <!--<div class="card-options">
                                        <button class="btn btn-sm" onclick="edit_run('<?php //echo $_GET['id'];?>')"><i
                                                class="fa fa-pencil-square-o mr-1"></i>แก้ไขยอดขาว</button>
                                        <button class="btn btn-danger btn-sm"
                                            onclick="edit_red('<?php //echo $_GET['id'];?>')"><i
                                                class="fa fa-pencil-square-o mr-1"></i>แก้ไขยอดแดง</button>
                                ...
                                <div class="card-body">
                                    ...
                                    $data = $mysqli->query("SELECT * FROM scb_transaction_".$_GET['id']." WHERE DATE(`trans_datetime`) LIKE ? ORDER BY trans_no DESC",$date)->fetchAll();
                                            ...
                                            <tbody>
                                                ...
                                                foreach($data as $result){
                                                    ...
                                                    echo'<tr>';
                                                        ...
                                                        echo '<td><a href="/office69/member/list?acc='.$result['trans_used_username'].'" class="text-warning">'.$result['trans_used_username'].'</a></td>';
                                                        ...
                                                        if($can_use AND ($result['trans_status'] != 1) AND ($result['trans_status'] != 6)){
                                                            ...
                                                            if(isset($result['trans_used_by']))
                                                                ...
                                                                echo '<button class="btn btn-sm bg-gray" onclick="use_bank(\''.$_GET['id'].'\',\''.$result['trans_no'].'\')"> ยังไม่ได้ใช้งาน </button>';
...
?>
    ...
    <script>
        ...
        var edit_red = (id) => {
            $.post('api/edit_red.php', {
                'id': id
            }, function (data) {
                swal({
                    title: "Success",
                    text: "แก้ไขยอดแดงสำเร็จ",
                    type: "success"
                });
            });
        };

        var use_bank = (id, trans_no) => {
